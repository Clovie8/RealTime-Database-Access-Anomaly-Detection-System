...
Configuration Tables: (APP_USERS, ALLOWED_IPS) define the rules.

Log Tables: (SECURITY_ALERTS, AUDIT_HISTORY) store the immutable evidence.

3. Data Flow Diagram

Event: User initiates LOGIN or UPDATE.

Intercept: Trigger captures the event payload (User, IP, Time, Data).
...
Analytics: Background views (V_SEC_ANALYTICS_FULL) aggregate logs into risk scores using Window Functions.

4. Security & Compliance Features

Immutable Logging: Alerts are written via independent transactions.
