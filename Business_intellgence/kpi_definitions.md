...
* **Calculation:**
    > $$(Count\ of\ SECURITY\_ALERTS\ /\ (Count\ of\ ACCESS\_LOG\ +\ Count\ of\ SECURITY\_ALERTS))\ *\ 100$$
...
* **Calculation:**
    > $$AVG(Time\_Difference(Alert\_N,\ Alert\_N-1))$$
...
* **Calculation:**
    ```sql
    SELECT TO_CHAR(alert_time, 'HH24'), COUNT(*) 
    FROM SECURITY_ALERTS 
    GROUP BY TO_CHAR(alert_time, 'HH24') 
    ORDER BY COUNT(*) DESC;
    ```
